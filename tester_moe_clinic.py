...
def summary(model, loader, n_classes):
    ...
    for batch_idx, (data, label, clinic) in enumerate(loader):
        ...
        with torch.inference_mode():
            ...
            logits, Y_prob, Y_hat, A_raw, results_dict, gate_weights = model[1]([image_features, clinic])
...
if __name__ == "__main__":
    save_dir = './results/TCH_all_clinic/'
    csv_path = './dataset_csv/TCH_with_clinical.csv'
    # data_dir = '/data/ceiling/data/DLBCL/Centers/BH_TCH/uni_scales/'
    data_dir = '/data/ceiling/data/DLBCL/Centers/BH_TCH/uni_scales/'
    weight_dir = './save_weights/Clinic_5fold_mutation_896_moe_focal/'
    ...
    model = [MultiscaleMoEAMIL(n_classes=n_classes).cuda(), ModelWithClinic().cuda()]
    folds = [0, 1, 2, 3, 4]
    ...
    for ckpt_idx in range(len(ckpt_paths)):
        ...
        model[0].load_state_dict(torch.load('./save_weights/BH_TCH_5fold_mutation_896_moe_bce555/s_{}_checkpoint_10.pt'.format(ckpt_idx)))
        model[1].load_state_dict(torch.load(ckpt_paths[ckpt_idx]))
