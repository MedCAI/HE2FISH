...
def gate_entropy_loss(gate_weights):
    # gate_weights: list of (batch_size, num_experts)
    eps = 1e-8
    entropy = 0
    for gw in gate_weights:
        # 对每个样本，每个gate输出算entropy
        ent = - (gw * (gw + eps).log()).sum(dim=1)  # (batch_size,)
        entropy += ent.mean()
    entropy = entropy / len(gate_weights)  # 对类别求平均
    return entropy
...
def summary(model, loader, n_classes):
    ...
    for batch_idx, (data, label, coords) in enumerate(loader):
        ...
        with torch.inference_mode():
            logits, Y_prob, Y_hat, A_raw, results_dict, gate_weight_list = model(data)
...
def train_abmil(datasets,
    ...
    model = MultiscaleMoEAMIL(n_classes=n_classes)
    model.load_state_dict(torch.load('./save_weights/BH_TCH_5fold_mutation_896_moe_bce555/s_{}_checkpoint_10.pt'.format(fold)))
    _ = model.to(device)
...
def train_loop(epoch, model, loader, optimizer, n_classes, writer, loss_fn):
    ...
    for batch_idx, (data, label, coords) in enumerate(loader):
        ...
        entropy_loss = gate_entropy_loss(gate_weight_list)
    
        loss = loss_bag + 0.01 * entropy_loss
        loss_bag_value = loss_bag.item()
        loss_gate_value = entropy_loss.item()
        ...
        if (batch_idx + 1) % 20 == 0:
            print('batch {}, loss: {:.4f}, loss_bag: {:.4f}, entropy_loss: {:.4}, label: {}, bag_size: {}'.format(
                batch_idx, loss_value, loss_bag_value, loss_gate_value, label.cpu().numpy()[0], data.size(0)
            ))
...
if __name__ == "__main__":
    ...
    csv_path = './dataset_csv/five.csv'
    data_dir = '/data/ceiling/data/DLBCL/Centers/BH_TCH/uni_scales/'
    split_path = '/data/ceiling/workspace/CLAM-master/splits/Five/'
    save_dir = './save_weights/Five_5fold_mutation_896_moe_10/'
